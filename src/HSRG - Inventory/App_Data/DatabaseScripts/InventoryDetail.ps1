 ...
 ## Gathers preliminary info and loops it for each computer ##
    ...
    foreach ($Computer in $ComputerName){
        ...
        ## Invokes gathering Hardware Information ##
            ...
            $drivespace = Get-WmiObject win32_logicaldisk -Computer $Computer | select @{Label='Drive';Expression={$_.DeviceID}}, @{LABEL='Free';EXPRESSION={"{0:N2}" -f($_.freespace/1GB)} },@{LABEL='Total';EXPRESSION={"{0:N2}" -f($_.Size/1GB)} } | ConvertTo-Json
...
Invoke-SQLiteBulkCopy -DataTable $dtable -DataSource $DataSource -Table InventoryDetail -Confirm:$false
