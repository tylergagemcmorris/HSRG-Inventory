 ...
 ## Gathers preliminary info and loops it for each computer ##
    ...
    foreach ($Computer in $ComputerName){
        ...
        ## Invokes gathering Hardware Information ##
            $bios = Get-WmiObject win32_logicaldisk -Computer $Computer | select @{Label='Drive Letter';Expression={$_.DeviceID}}, @{Label='Label';Expression={$_.VolumeName}}, @{LABEL='Free Space - GB';EXPRESSION={"{0:N2}" -f($_.freespace/1GB)} },@{LABEL='Total Space - GB';EXPRESSION={"{0:N2}" -f($_.Size/1GB)} }

                    $Obj = New-Object psobject -Property @{
                        'ComputerID' = $Computer
                        'Drive Letter' = $bios.'Drive Letter' | Out-String
                        'Label' = $bios.Label | Out-String
                        'Free Space - GB' = $bios.'Free Space - GB' | Out-String
                        'Total Space - GB' = $bios.'Total Space - GB' | Out-String
                          }
                        $Objs += $Obj
                    }            
...
#$Query = "CREATE TABLE DriveSpace (ComputerID TEXT PRIMARY KEY, 'Drive Letter' TEXT, Label TEXT, 'Free Space - GB' DECIMAL, 'Total Space - GB' DECIMAL)"
