 ...
 ## Gathers preliminary info and loops it for each computer ##
    ...
    foreach ($Computer in $ComputerName){
        ...
        ## Invokes gathering Hardware Information ##
            $bios = Get-WmiObject Win32_PhysicalMemory -ComputerName $Computer  | select BankLabel,DeviceLocator, @{n='Capacity';e={"{0:N2}" -f[Decimal]($_.Capacity/1GB)}},Manufacturer,PartNumber,SerialNumber,Speed


                    $Obj = New-Object psobject -Property @{
                        'ComputerID' = $Computer
                        'BankLabel' = $bios.BankLabel | Out-String
                        'DeviceLocator' = $bios.DeviceLocator | Out-String
                        'Capacity' = $bios.Capacity | Out-String
                        'Manufacturer' = $bios.Manufacturer | Out-String
                        'PartNumber' = $bios.PartNumber | Out-String
                        'SerialNumber' = $bios.SerialNumber | Out-String
...
## Selects Object 
$objs | Select-Object ComputerID, BankLabel,DeviceLocator,Capacity,Manufacturer,PartNumber,SerialNumber,Speed
...
#$Query = "CREATE TABLE MemoryInformation (ComputerID TEXT PRIMARY KEY, BankLabel TEXT, DeviceLocator TEXT, Capacity DECIMAL, Manufacturer TEXT, PartNumber TEXT, SerialNumber INTEGER, Speed INTEGER)"
