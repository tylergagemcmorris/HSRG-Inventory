 ...
 ## Gathers preliminary info and loops it for each computer ##
    ...
    foreach ($Computer in $ComputerName){
        ...
        ## Invokes gathering Hardware Information ##
            $BaseboardObj = Get-WmiObject Win32_BaseBoard -ComputerName $Computer |  Select Name, Manufacturer, Product, SerialNumber, Status
 	       
                    $Obj = New-Object psobject -Property @{
                        'ComputerID' = $Computer
                        'Name' = $BaseboardObj.Name
                        'Manufacurer' = $BaseboardObj.Manufacturer
                        'Product' = $BaseboardObj.Product
                        'Serial Number' = $BaseboardObj.SerialNumber
                        'Status' = $BaseboardObj.Status
                          }
                        $Objs += $Obj
                    }            

## Selects Object 
$objs | Select-Object 'ComputerID', 'Name', 'Manufacurer', 'Product', 'Serial Number', 'Status'
...
#$Query = "CREATE TABLE MotherBoardInformation (ComputerID TEXT PRIMARY KEY, Name TEXT, Manufacurer TEXT, Product INTEGER, Serial Number TEXT, Status TEXT)"
#Invoke-SqliteQuery -DataSource $DataSource -Query $Query 


## Takes items pulled from object created and outputs them to datatable ##
$logdsk = $Objs | select ComputerID, Name, Manufacurer, Product, 'Serial Number', Status
